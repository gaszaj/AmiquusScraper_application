...
export const PaymentUpdateModal = ({
    ...
    useEffect(() => {
        ...
        async function hydrateExistingDiscount() {
            // default: allow entry
            ...
            const discountData = await getDiscountDetails(subscription.discountId);

            if (cancelled) return;

            if (discountData) {
                setDiscount(discountData);
                setDiscountCode(subscription.promoCode || "");
                setIsDiscountValid(true);
                setCanUseDiscount(false); // show "applied" UI
            } else {
                setDiscount(null);
        ...
        hydrateExistingDiscount();

        return () => {
            cancelled = true;
        };
    }, [subscription.discountId, subscription.promoCode, subscription.codeApplied]);
    ...
    const removeDiscount = () => {
        ...
        setIsDiscountValid(null);
        setCanUseDiscount(true);
        setDiscountCode(""); // or keep old in case they want to reapply
        toast({
            title: "Discount removed",
            description: "Promo code removed. You can apply a new one if you want.",
        });
    };
    ...
    const handleUpdatePayment = async () => {
        ...
        try {
            const updateData = {
                ...
                // ✅ backend expects promoCode string|null
                            ...
                            <div className="flex items-start justify-between gap-3 rounded-xl border p-3">
                                <div className="flex flex-col">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Code: <span className="font-mono">{discount.code}</span> • {formatDiscountLabel(discount)} off
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                        {formatCyclesText(discount.subscription_cycles ?? undefined)}
                                    </p>
                                </div>
